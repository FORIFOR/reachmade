 ...
 * All packaged films are concise edits of real product recordings. Playback speed is unchanged.
 */
(() => {
  'use strict';
  ...
  const words = ja ? {
    play:'再生', pause:'一時停止', loading:'読み込み中', expand:'大きく見る', close:'閉じる',
    preview:'無音プレビュー', recorded:'実録画のサイト用編集版',
    error:'動画を読み込めませんでした。', original:'製品サイトで詳しく見る',
    hint:'実録画を約13秒に編集しています。再生速度は変えていません。停止・拡大できます。',
    motion:'自動再生はオフです。実録画の短い編集版を再生ボタンからご覧ください。',
    homeAuto:'最初の実録画1本だけを無音再生します。再生速度は変えていません。ほかは再生ボタンからご覧ください。',
  } : {
    play:'Play', pause:'Pause', loading:'Loading', expand:'View larger', close:'Close',
    preview:'Silent preview', recorded:'Website edit of the real recording',
    error:'The film could not be loaded.', original:'View the product site',
    hint:'A concise edit of the real recording, with playback speed unchanged. Pause or open it larger.',
    motion:'Autoplay is off. Use Play to watch the concise edit of the real recording.',
    homeAuto:'Only the first real recording plays automatically. Playback speed is unchanged; use Play for the others.',
  };
  const films = [
    ...
    {id:'oathra',name:'Oathra',repo:'oathra',start:0,end:90,
      ...
      note:ja?'シミュレーター実録画の編集版。実電話や店舗システムへの登録完了を示すものではありません。':'Edited real simulator recording; not a live call or proof of a booking-system entry.'},
    {id:'aisecure',name:'AI Secure',repo:'AISecure',start:0,end:90,
      demo:'https://forifor.github.io/AISecure/',
      note:ja?'合成データによる実画面録画の編集版。実環境の監視・遮断は行いません。':'Edited real UI recording on synthetic data; no live monitoring or blocking.'},
    {id:'agent-team',name:'Agent Team',repo:'Multibot',start:0,end:90,
      demo:'https://forifor.github.io/Multibot/ja/#record',
      note:ja?'実モデル実行の録画を短く編集。元の実行は約24分で、出典照合を残して部分完了です。':'Concise edit of a real model run. The original run was about 24 minutes and ended partial with source verification unfinished.'},
  ...
  style.rel='stylesheet'; style.href='/assets/product-films.css?v=20260916-premium2'; document.head.append(style);
  ...
  const poster = film => `/media/products/${film.id}.jpg`;
  ...
  function openFilm(s,from) {
    ...
    fullError.hidden=true;full.poster=poster(s.film);full.src=source(s.film);full.muted=true;full.load();
  ...
  for (const {film,host} of targets) {
    ...
    video.preload='none';video.poster=poster(film);video.setAttribute('aria-label',`${film.name}: ${words.preview}`);
    ...
    const status=el('span','rm-film-state',words.recorded);
    ...
    const s={film,host,frame,stage,video,progress,toggle,status,error,fallback,poster:poster(film),ratio:0,manual:null,manualOrder:0,want:false,starting:false,failed:false,autoEligible:!!host.closest('.hero')};
