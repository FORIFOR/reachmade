...
if (form) {
  ...
  let available=false, busy=false, checking=false, previous=null;
  ...
  form.addEventListener('submit',async event=>{
    ...
    try {
      ...
      if(r.status===201&&/^AM-[A-F\d]{8}$/.test(answer.receipt||'')){
        form.reset();previous=null;result.dataset.receipt=answer.receipt;
        say(t(`相談を受け付けました。受付番号：${answer.receipt}。自動返信メールはありません。`,`Inquiry received. Receipt: ${answer.receipt}. No automatic email is sent.`));
      }else if(r.status===429){say(t('受付件数の上限に達しています。入力は残しています。時間を置いて再試行してください。','The intake limit was reached. Your input is retained. Please retry later.'));}
      else if(r.status===400||r.status===413){say(t('入力の長さ・メールアドレス・同意欄を確認してください。内容は残しています。','Check the message length, email and consent. Your input is retained.'));}
      else if(r.status===409){say(t('前回の送信内容と一致しません。受付が重複しないよう、既存の窓口へお問い合わせください。','This differs from the previous submission. Use the existing contact route to avoid a duplicate inquiry.'));}
      else throw Error('unconfirmed');
    }catch{say(t('受付結果を確認できませんでした。入力は残しています。同じ内容で再試行すると、同じ受付IDを使って重複を防ぎます。','Receipt could not be confirmed. Your input is retained. Retrying unchanged content reuses the same ID to prevent a duplicate.'));}
    finally{busy=false;fields.disabled=false;submit.disabled=!available;reconnect.disabled=false;form.removeAttribute('aria-busy');}
  });
  check();
}
