...
export function renderHero(products, lang) {
  ...
  return `<section class="hero container lab-hero rm-hero"><div class="rm-hero-grid"><div class="rm-hero-copy"><p class="rm-hero-badge"><span aria-hidden="true">●</span> ${esc(badge)}</p><p class="eyebrow">REACHMADE / INDEPENDENT AI STUDIO</p><h1>${t('思いついたら、<br>使えるかたちに。', 'From an idea.<br>To something real.')}</h1><p class="rm-hero-lead">${t('メモを、計画に。会話を、タスクに。<br>録画を、伝わる紹介素材に。', 'Notes into plans. Conversations into tasks.<br>Recordings into launch material.')}</p><div class="rm-hero-actions"><a class="rm-primary" data-try-open href="#studio-player">${t('ブラウザで試す', 'Try it in your browser')} <span aria-hidden="true">↓</span></a><a class="rm-secondary" href="#explore">${t('6つの製品を見る', 'See all six products')} <span aria-hidden="true">↓</span></a></div><p class="rm-hero-requirement">${t('登録なし・外部送信なし。編集にはJavaScriptが必要です。実アプリのGenieは別途Macとローカル設定が必要です。', 'No account, nothing sent. Editing needs JavaScript. The Genie app itself needs a Mac and local setup.')}</p><ul class="rm-hero-evidence">${evidence.map(item => `<li>${esc(item)}</li>`).join('')}</ul><ul class="rm-hero-facts">${facts.map(([value, label]) => `<li><b>${esc(value)}</b><span>${esc(label)}</span></li>`).join('')}</ul></div><div class="rm-hero-stage">${renderSampleFilm(lang)}</div></div><div class="rm-hero-nav">${renderTaskPicker(list, lang)}</div><div class="rm-proof"><div class="rm-proof-head"><p class="eyebrow">${t('REAL RECORDING / GENIE', 'REAL RECORDING / GENIE')}</p><p>${t('実際に何ができるか、録画と保存された成果物で確かめてください。', 'See what the app does in a real recording, then open its saved artifact.')}</p></div>${renderRecordedResult(lang)}</div>${renderTryNow(lang)}<div class="rm-bridge"><span>${t('「使えるかたち」を、まずは手元で。', 'Something real, right in your browser.')}</span><p>${t('実演、保存した作例、導入手順まで。6つの製品を、実物から選べます。', 'Recording, saved artifact, and setup. Choose between six products, starting with the real work.')}</p><a href="#explore">${t('プロダクトを見る', 'Meet the products')} <span aria-hidden="true">↓</span></a></div></section>${renderMarquee(list, lang)}`;
}

/** The recording of this repository's own sample, made by operating it. Never autoplays. */
export function renderSampleFilm(lang) {
  check(lang);
  const ja = lang === 'ja', t = (a, b) => (ja ? a : b);
  const suffix = ja ? '' : '-en';
  return `<figure class="rm-film"><video class="rm-film-video" controls playsinline preload="none" poster="/media/derived/sample/sample-poster${suffix}.jpg" aria-label="${t('操作サンプルの収録', 'Recording of the guided sample')}"><source media="(max-width:700px)" src="/media/derived/sample/sample-9x16${suffix}.mp4" type="video/mp4"><source src="/media/derived/sample/sample-16x9${suffix}.mp4" type="video/mp4">${t('この動画は再生できません。同じ操作はサンプルから手で試せます。', 'This video cannot play here. The same steps are available by hand in the sample.')}</video><figcaption class="rm-film-note"><b>${t('このサイトの操作サンプルを、実際に操作して収録', 'The guided sample on this site, recorded by operating it')}</b><span>${t('架空データ・AI実行なし・外部送信なし。字幕は後から付けています。実ブラウザで撮ったコマを12fpsで並べたもので、早送りや待ち時間の削除はしていません。', 'Fictional data, no AI run, nothing sent. Captions were added afterwards. Frames captured in a real browser and assembled at 12fps; nothing is sped up and no waiting was cut.')}</span></figcaption></figure>`;
}

/** The shortest path to something the visitor keeps. States its own requirement. */
export function renderTryNow(lang) {
  check(lang);
  const ja = lang === 'ja', t = (a, b) => (ja ? a : b);
  const steps = ja
    ? [['01', '例文を書き換える', '用意された文章を、自分の内容に編集します。'],
       ['02', '内容を確認する', '書き換えた文章をそのまま確認します。練習問題はスキップできます。'],
       ['03', 'Markdownで持ち出す', '.mdとしてダウンロードし、あとで読み戻せます。']]
    : [['01', 'Edit the example', 'Replace the prepared text with your own.'],
       ['02', 'Review what you wrote', 'Read your own text back. The practice question is skippable.'],
       ['03', 'Keep it as Markdown', 'Download a .md file and reopen it later.']];
  return `<div class="rm-try" id="try"><div class="rm-try-head"><p class="eyebrow">${t('いますぐ試す', 'TRY IT HERE')}</p><h2>${t('手元に残るものを、<br>先に作る。', 'Make something<br>you keep first.')}</h2><p>${t('ブラウザの中だけで動くサンプルです。AI実行も外部送信もありません。残るのは、あなたが書いた.mdファイル1つ。', 'A sample that runs only inside your tab. No AI run, nothing sent. What is left is one .md file you wrote.')}</p></div><ol class="rm-try-steps">${steps.map(([n, title, body]) => `<li><span class="rm-try-index">${n}</span><strong>${esc(title)}</strong><span>${esc(body)}</span></li>`).join('')}</ol><div class="rm-try-foot"><a class="rm-secondary" data-try-open href="#studio-player">${t('サンプルを開く', 'Open the sample')} <span aria-hidden="true">↓</span></a><span>${t('編集にはJavaScriptが必要です。JavaScriptなしでも、実録画と保存済みの成果物は開けます。', 'Editing needs JavaScript. Without it, the recording and the saved artifact still open.')}</span></div></div>`;
}
