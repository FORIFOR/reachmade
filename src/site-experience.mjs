...
export function improveCopy(original) {
  ...
  copy.ja.privacySections[1][1] = 'このサイト内の問い合わせは、画面に表示する運営者の受付へ送信します。製品サイト・GitHubへのリンクには外部サービスが含まれます。移動後は、その送信先と情報の取り扱いを確認してください。';
  copy.en.privacySections[1][1] = 'On-site inquiries use the operator’s intake described beside the form. Some product links and GitHub open external services. Check their destination and privacy information before submitting there.';
  return copy;
}
