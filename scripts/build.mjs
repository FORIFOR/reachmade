...
function productIllustration(p,lang) {
 ...
 return `<div class="product-notation notation-${p.id}"><div class="notation-top"><span>${p.discipline}</span><span>${p.index} / 06</span></div><div class="notation-center">${icon(p.id)}<span class="notation-name">${p.name}</span><span class="notation-rule"></span><div class="notation-flow">${parts.map((s,i)=>`<span>${e(s)}</span>${i<parts.length-1?'<i aria-hidden="true">↗</i>':''}`).join('')}</div></div><div class="notation-bottom"><span>${sub}</span><span aria-hidden="true">↗</span></div></div>`;
}
function productRow(p,lang,full=false) {
 const c=copy[lang],t=p[lang];
 return `<article class="project-row${full?' project-full':''}" id="${p.id}" data-category="${p.category}"><div class="project-visual">${productIllustration(p,lang)}</div><div class="project-content"><div class="project-heading"><span class="eyebrow">${p.index} — ${p.name}</span><span class="status-label">${t.status}</span></div><h${full?'2':'3'}>${t.headline}</h${full?'2':'3'}><p>${t.description}</p><div class="project-actions">${link(p.demo,c.realDemo,'button button-outline')}${link(p.repo,'GitHub','text-link')}</div><p class="project-scope">${t.scope}</p>${full?`<details class="project-detail"><summary>${lang==='ja'?'検証・導入条件を確認する':'Evidence & reuse conditions'}<span aria-hidden="true">＋</span></summary><div><h3>${c.proof}</h3><p>${t.proof}</p><h3>${lang==='ja'?'再利用の条件':'Reuse conditions'}</h3><p>${t.license}</p><h3>${lang==='ja'?'この技術を活かせる相談':'Potential project scope'}</h3><p>${t.consult}</p><div class="project-detail-links">${link(p.source,c.readSource)}${link(p.evidence,c.evidence)}<a class="text-link" href="${href(lang,'contact')}?product=${p.id}">${c.inquiry}${arrow}</a></div></div></details>`:''}</div></article>`;
}
...
<section class="product-ribbon" aria-label="${lang==='ja'?'6つの自主開発プロダクト':'Six independent products'}"><div class="container ribbon-inner"><span class="ribbon-title">BUILT HERE</span>${products.map(p=>`<a href="${href(lang,'products')}#${p.id}">${p.name}<sup>${p.index}</sup></a>`).join('')}</div></section>
...
function productsPage(lang) {
 const c=copy[lang];
 return `${pageIntro('PRODUCTS / 01—06',c.productsTitle,c.productsLead)}<section class="container product-directory"><div class="filter-bar"><div class="product-filters" role="group" aria-label="${lang==='ja'?'分野で絞り込む':'Filter by category'}">${c.filters.map(([value,label],i)=>`<button type="button" data-filter="${value}" aria-pressed="${i===0}">${label}</button>`).join('')}</div><p class="filter-count" role="status" aria-live="polite" data-count-suffix="${c.countSuffix}">6 ${c.countSuffix}</p></div><p class="source-notice">${c.productNotice}</p>${products.map(p=>productRow(p,lang,true)).join('')}</section>${contactCTA(lang,true)}`;
}
