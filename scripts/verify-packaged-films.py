"""Check premium built pages, the signature reel and packaged recordings via local Wrangler; never deploy."""
...
def assert_h264_silent_720(metadata, duration_min, duration_max):
    ...
    assert duration_min <= duration <= duration_max, duration
    return duration
...
def main():
    ...
    report = {'scope': 'Premium website edits, six-product signature reel, posters and built pages through local Wrangler; not production or Safari', 'media': [], 'signature': {}, 'pages': [], 'errors': []}
    by_id = {}
    ...
    assert signature['duration'] == 12
    assert signature['secondsPerProduct'] == 2
    ...
    for item in manifest['recordings']:
        ...
        try:
            ...
            duration = assert_h264_silent_720(metadata, 12.5, 13.5)
            ...
            report['errors'].append(item['id'] + ': ' + str(error))

    try:
        signature_target = ROOT / 'dist' / signature['path'].lstrip('/')
        signature_poster = ROOT / 'dist' / signature['poster'].lstrip('/')
        signature_raw = signature_target.read_bytes()
        signature_poster_raw = signature_poster.read_bytes()
        assert hashlib.sha256(signature_raw).hexdigest() == signature['sha256']
        assert len(signature_raw) == signature['bytes'] < 25 * 1024 * 1024
            ...
            for language in ['', 'en/']:
                for sub in ['', 'products/', 'services/', 'work/', 'about/', 'contact/', 'privacy/']:
                    ...
                    try:
                        ...
                        if sub == '':
                            assert page.locator('.hero .rm-signature-stage').count() == 1
                            assert page.locator('.hero .rm-signature-video').count() == 1
                            assert page.locator('.hero .rm-signature-video[src]').count() == 0
                            assert page.locator('.hero .rm-signature-video').evaluate('(v)=>v.paused')
                            assert page.locator('.hero .rm-signature-video').get_attribute('poster') == '/assets/reachmade-signature.jpg'
