...
def assert_h264_silent_720(metadata,minimum,maximum):
    video=[s for s in metadata['streams'] if s.get('codec_type')=='video']
    assert len(video)==1 and not [s for s in metadata['streams'] if s.get('codec_type')=='audio']
    assert video[0]['codec_name']=='h264'
    assert (video[0]['width'],video[0]['height'])==(1280,720)
    duration=float(metadata['format']['duration']);assert minimum<=duration<=maximum,duration
    return duration

def delivery(item,raw,allow_full=False):
    with urllib.request.urlopen(urllib.request.Request(BASE+item['path'],method='HEAD'),timeout=15) as r:
        assert r.status==200 and r.headers.get_content_type()=='video/mp4'
    with urllib.request.urlopen(urllib.request.Request(BASE+item['path'],headers={'Range':'bytes=0-63'}),timeout=15) as r:
        assert r.status in ((200,206) if allow_full else (206,))
        assert r.headers.get_content_type()=='video/mp4'
        assert r.read()==(raw[:64] if r.status==206 else raw)
...
def main():
    ...
    for item in manifest['recordings']:
        ...
        try:
            ...
            target=ROOT/'dist'/item['path'].lstrip('/');raw=target.read_bytes()
            assert hashlib.sha256(raw).hexdigest()==item['sha256']
            assert len(raw)==item['bytes']<25*1024*1024
            poster=(ROOT/'dist'/item['poster'].lstrip('/')).read_bytes()
            assert poster[:2]==b'\xff\xd8' and poster[-2:]==b'\xff\xd9'
            assert (ROOT/'dist/assets/products'/f"{item['id']}.jpg").read_bytes()==poster
            metadata=probe_video(target);duration=assert_h264_silent_720(metadata,12.5,13.5)
            delivery(item,raw)
            result.update(bytes=len(raw),poster_bytes=len(poster),duration=duration,metadata=metadata,head_range_poster=True)
        except Exception as error:
            report['errors'].append(item['id']+': '+str(error))
    try:
        target=ROOT/'dist'/signature['path'].lstrip('/');raw=target.read_bytes()
        assert hashlib.sha256(raw).hexdigest()==signature['sha256']
    ...
    with sync_playwright() as pw:
        ...
        for item in manifest['recordings']:
            try:
                ...
                frame.locator('.rm-film-toggle').click();assert frame.locator('video').evaluate('v=>v.paused')
