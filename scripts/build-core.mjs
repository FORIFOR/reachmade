...
import { copy as originalCopy } from '../src/copy.mjs';
import { improveCopy, productNavigation, contactDestination, productIndex, caseStudies, serviceExamples, profileProof } from '../src/site-experience.mjs';
const copy = improveCopy(originalCopy);
...
function identityStudy(c,lang) {
 ...
 const featuredLink=featured.labSite?link(productNavigation(featured,lang).site,lang==='ja'?'製品サイトを見る':'Open the product site','text-link'):'';
...
function footer(lang,config) {
 const c=copy[lang];
 return `<footer class="site-footer"><div class="container footer-main"><div><a class="brand footer-brand" href="${href(lang)}">${mark()}<span>Reachmade<span class="brand-lab">Lab</span></span></a><p>${c.footerBrand}<br>${c.footerNote}</p></div><nav aria-label="${lang==='ja'?'フッターナビゲーション':'Footer navigation'}"><a href="${href(lang,'products')}">${c.nav[0]}</a><a href="${href(lang,'services')}">${c.nav[1]}</a><a href="${href(lang,'about')}">${c.nav[3]}</a>${link(config.github,'GitHub')}</nav></div><div class="container footer-bottom"><small>© 2026 Reachmade Lab / Shuhei Horio</small><div><a href="${href(lang,'privacy')}">${c.privacy}</a><a href="${href(lang==='ja'?'en':'ja')}">${lang==='ja'?'English':'日本語'}</a><a href="#top" class="to-top" aria-label="${lang==='ja'?'ページ先頭へ':'Back to top'}">↑</a></div></div></footer>`;
}
...
function productRow(p,lang,full=false) {
 const c=copy[lang],t=p[lang],n=productNavigation(p,lang);
 return `<article class="project-row${full?' project-full':''}" id="${p.id}" data-category="${p.category}"><div class="project-visual">${productIllustration(p,lang)}</div><div class="project-content"><div class="project-heading"><span class="eyebrow">${p.index} — ${p.name}</span><span class="status-label">${t.status}</span></div><h${full?'2':'3'}>${t.headline}</h${full?'2':'3'}><p>${t.description}</p><div class="project-actions">${p.labSite?link(n.site,c.productSite,'button button-outline'):''}${link(n.demo,n.demoLabel,'button button-outline')}${link(p.repo,'GitHub','text-link')}</div><p class="project-scope">${t.scope}</p>${full?`<details class="project-detail"><summary>${lang==='ja'?'検証・導入条件を確認する':'Evidence & reuse conditions'}<span aria-hidden="true">＋</span></summary><div><h3>${c.proof}</h3><p>${t.proof}</p><h3>${lang==='ja'?'再利用の条件':'Reuse conditions'}</h3><p>${t.license}</p><h3>${lang==='ja'?'この技術を活かせる相談':'Potential project scope'}</h3><p>${t.consult}</p><div class="project-detail-links">${link(p.source,c.readSource)}${link(p.evidence,c.evidence)}<a class="text-link" href="${href(lang,'contact')}?product=${p.id}">${c.inquiry}${arrow}</a></div></div></details>`:''}</div></article>`;
}
function compactProduct(p,lang) {
 const c=copy[lang],t=p[lang];
 return `<article class="compact-product"><div class="compact-product-meta"><span class="eyebrow">${p.index} — ${e(p.name)}</span><span class="status-label">${e(t.status)}</span></div><h3>${e(t.short)}</h3><p>${e(t.description)}</p><div class="compact-product-actions">${p.labSite?link(productNavigation(p,lang).site,c.productSite):''}${link(p.repo,'GitHub')}</div></article>`;
}
...
function productsPage(lang) {
 const c=copy[lang];
 return `${pageIntro(`PRODUCTS / 01—${productTotal}`, c.productsTitle,c.productsLead)}${productIndex(products,lang)}<section class="container product-directory"><div class="filter-bar"><div class="product-filters" role="group" aria-label="${lang==='ja'?'分野で絞り込む':'Filter by category'}">${c.filters.map(([value,label],i)=>`<button type="button" data-filter="${value}" aria-pressed="${i===0}">${label}</button>`).join('')}</div><p class="filter-count" role="status" aria-live="polite" data-count-suffix="${c.countSuffix}">${products.length} ${c.countSuffix}</p></div><p class="source-notice">${c.productNotice}</p>${products.map(p=>productRow(p,lang,true)).join('')}</section>${contactCTA(lang,true)}`;
}
function services(lang) {
 const c=copy[lang];
 return `${pageIntro('FOR COMPANIES',c.servicesTitle,c.servicesLead)}<section class="container service-introduction"><span class="eyebrow">FROM PROTOTYPE TO PRACTICE</span><p>${c.serviceIntro}</p></section>${serviceExamples(lang)}<section class="container service-details">${c.serviceCards.map(x=>`<article id="service-${x[0]}" class="service-item"><div class="service-number">${x[0]}</div><div><h2>${x[1]}</h2><p>${x[2]}</p><div class="service-deliverables"><h3>${c.deliverables}</h3><p>${x[3]}</p></div><p class="related-work">${c.related} — <a href="${href(lang,'products')}">${x[4]}</a></p></div><a class="round-link" href="${href(lang,'contact')}?service=${x[0]}" aria-label="${x[1]} — ${c.contact}">↗</a></article>`).join('')}</section><section class="process-section"><div class="container"><p class="eyebrow">THE PROCESS</p><h2>${c.processTitle}</h2><div class="process-grid">${c.process.map((x,i)=>`<article><span>0${i+1}</span><h3>${x[0]}</h3><p>${x[1]}</p></article>`).join('')}</div></div></section><section class="container engagement-scope"><h2>${c.scopeTitle}</h2><p>${c.scopeText}</p></section><section class="container faq"><p class="eyebrow">QUESTIONS, ANSWERED</p><h2>${lang==='ja'?'相談の前に。':'Before we start.'}</h2>${c.faq.map(x=>`<details><summary>${x[0]}<span aria-hidden="true">＋</span></summary><p>${x[1]}</p></details>`).join('')}</section>${contactCTA(lang,true)}`;
}
function work(lang,config) {
 const c=copy[lang];
 return `${pageIntro('WORK NOTES',c.workTitle,c.workLead)}${caseStudies(products,lang)}<section class="container work-records"><p class="source-notice">${c.workNote}<br><span>${lang==='ja'?'資料確認日':'Source review date'}: ${config.checkedAt}</span></p>${products.map(p=>`<article class="work-record" id="${p.id}"><div class="work-record-name"><span class="eyebrow">NOTE ${p.index}</span><h2>${p.name}</h2><span class="status-label">${p[lang].status}</span></div><div class="work-record-body"><h3>${c.proof}</h3><p>${p[lang].proof}</p><h3>${c.limits}</h3><p>${p[lang].scope}</p><div class="record-links">${link(p.evidence,c.evidence)}${link(p.demo,p[lang].demoLabel)}${link(p.source,c.readSource)}</div></div></article>`).join('')}</section>${contactCTA(lang,true)}`;
}
function about(lang,config) {
 ...
 return `${pageIntro('ABOUT REACHMADE',c.aboutTitle,c.aboutLead)}<section class="container about-story"><div class="about-emblem" aria-hidden="true">${mark()}<span>REACH<br>MADE.</span></div><div class="about-prose"><p>${c.aboutBody}</p><p>${c.aboutBody2}</p><div class="name-note"><h2>Reach / made</h2><p>${c.identity}</p></div></div></section><section class="container founder"><div><p class="eyebrow">THE PERSON BEHIND THE WORK</p><h2>${e(config.owner)}</h2><span>${c.ownerRole}</span></div><div><p>${projectTrail}</p>${link(config.github,lang==='ja'?'FORIFORのGitHubを見る':'Explore FORIFOR on GitHub','button button-outline')}<p class="source-notice">${c.aboutNotice}</p></div></section>${profileProof(lang)}<section class="principles container"><p class="eyebrow">HOW WE BUILD</p><h2>${c.philosophy}</h2><div class="principle-grid">${c.principles.map((x,i)=>`<article><span class="principle-number">0${i+1}</span><h3>${x[0]}</h3><p>${x[1]}</p></article>`).join('')}</div></section>${contactCTA(lang,true)}`;
}
function contact(lang,config) {
 const c=copy[lang];
 const dest=contactDestination(config,lang);
...
function shell(body,lang,page,config) {
 const c=copy[lang],titleBase=page==='home'?strip(c.tagline):({products:lang==='ja'?'プロダクト':'Products',services:lang==='ja'?'企業向けAI実装・FDE支援':'AI implementation for companies',work:lang==='ja'?'開発・検証記録':'Work notes',about:lang==='ja'?'ラボについて':'About',contact:lang==='ja'?'共同開発の相談':'Discuss a project',privacy:lang==='ja'?'プライバシー':'Privacy','404':'404'})[page];
