...
- `python tests/browser-check.py`: **89 checks passed / 0 failed**（2026-09-15の記録）。Chromium 144.0.7559.96。実画面プレビューとCTA文言を更新した後の再実行は、この環境にPlaywrightがないため未実施。
...
75件は15ページ×5画面幅（320 / 390 / 768 / 1440 / 1920px）。メイン表示、H1、横はみ出し、JavaScriptエラーを確認。今回追加した実画面プレビューは、静的テストで全6件のローカルアセット参照を確認。
