...
## 2回目のゲート実行 — 初レビュー範囲（2026-09-19）

1回目はファーストビューだけを見ていた。今回は**これまで誰も見ていなかった範囲**を対象にした。

### 増やした証拠

トップの全体（日英、1440×8400）、製品ページ全体、モバイル全体（390×12200）、
768 / 960 / 1024 / 1920px。ページ下部は全体撮影から等倍クロップを切り出してレビューに渡した。

### レビュー 3巡

round 1 **revise**（P1×1、P2×4）→ round 2 **keep** → round 3 **keep**（証拠を撮り直して再確認）。

**P1は「直す」ではなく「消す」で解決した。** 製品ページに追加した進行レールは、上の実演パネル
（5段・操作系つき）と下の既存セクション「この実演で見ること」（同一内容）に挟まれた3つ目の重複で、
どちらの再生を押せばよいか画面から決まらない状態を作っていた。体裁を整えるのではなく、
コードとCSSごと削除した。製品ページに残したのは既存UIストーリーの改善だけ。

### 撮影方法の欠陥が2件見つかった

どちらも**誤った「確認済み」を生む**性質のもので、レビューがなければ気づかなかった。

1. **登場アニメーションの途中フレームが写る。** `lab-arrive`は`opacity:.6`起点で`both`指定のため、
   フレームが進まないheadlessでは開始状態で固まる。英語ページの締めCTAが薄いという誤った証拠が出た。
   → 全体撮影を`prefers-reduced-motion: reduce`に変更。実測で締めCTAの文字色は日英とも`(34,43,37)`。
2. **アンカー付きURL（`/#faq`）が黙ってページ最上部を返す。** `scroll-behavior:smooth`が
   フレームを必要とするため。「FAQを確認した」という誤った証拠になりかけた。
   → 全体撮影からの切り出しに変更。

いずれも`.claude/rules/ui-quality.md`のEvidence contractに再発防止として書いた。

### ゲート結果

`status: pass` / checks `check` exit 0 / artifacts 16（画像15 + レビュー1）。

### この回で確認していないこと

FAQの開閉の実動作、JavaScript無効時のFAQ、320pxと各幅の折り返し以降、自走再生、
キーボード操作とフォーカス復帰、コントラストの第三者実測、タッチターゲット実寸、
実機iPhone Safari、reduced-motionが実ブラウザで効くか、利用者テスト。
